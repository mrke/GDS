...
with(burrow.surf,plot(humidity~date_time,type='l',col='red',ylim=c(0,100),xlim=c(as.numeric(as.POSIXct("2013-10-01",origin="1970-01-01")),as.numeric(as.POSIXct("2014-12-01",origin="1970-01-01"))),main=burrow.title,xlab="",ylab="relative humidity (%)",xaxt = "n")) # plot the data
axis.POSIXct(side = 1, x = burrow.surf$date_time,
...
burrow.mid<-burrow.mid[order(burrow.mid$date_time),] 
  ...
  with(burrow.mid,points(humidity~date_time,type='l',col='orange')) # plot the data
...
burrow.deep<-burrow.deep[order(burrow.deep$date_time),] 
  ...
  with(burrow.deep,points(humidity~date_time,type='l',col='blue')) # plot the data
grid(NA, NULL) # grid only in y-direction


predicted<-read.csv('microclimate/humid.csv')
tzone<-paste("Etc/GMT-",10,sep="") # doing it this way ignores daylight savings!
dates<-seq(ISOdate(2012,1,1,tz=tzone)-3600*12, ISOdate(2015,1,1,tz=tzone)-3600*13, by="hours")
dates<-subset(dates, format(dates, "%m/%d")!= "02/29") # remove leap years
predicted<-cbind(dates,predicted) # add dates on

with(predicted,plot(RH1.5cm*100~dates,type='l',col='red',ylim=c(0,100),xlim=c(as.numeric(as.POSIXct("2013-10-01",origin="1970-01-01")),as.numeric(as.POSIXct("2014-12-01",origin="1970-01-01"))),main="predicted",xlab="",ylab="relative humidity (%)",xaxt = "n")) # plot the data
axis.POSIXct(side = 1, x = soil.surfaces$date_time,
             at = seq(ISOdate(2013,07,011), ISOdate(2015,01,01), "months"), format = "%b %y",
             las = 2)
with(predicted,points(RH5cm*100~dates,type='l',col='orange')) # plot the data
with(predicted,points(RH15cm*100~dates,type='l',col='brown')) # plot the data
with(predicted,points(RH30cm*100~dates,type='l',col='light blue')) # plot the data
with(predicted,points(RH60cm*100~dates,type='l',col='blue')) # plot the data
with(predicted,points(RH100cm*100~dates,type='l',col='black')) # plot the data
