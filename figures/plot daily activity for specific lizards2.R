...
duskmin<-19
duskmax<-20
time_budget$timeday[(time_budget$hour>=dawnmin & time_budget$hour<dawnmax) | (time_budget$hour>=duskmin & time_budget$hour<duskmax)]<-"duskdawn"
time_budget$timeday[time_budget$hour>solmax]<-"day"
...
plotsolar<-cbind(time_budget$pred_solar,seq(1,nrow(time_budget)),time_budget$timeday)
plot(plotsolar[,1]~plotsolar[,2],type="b")
plotsolar2<-subset(plotsolar, plotsolar[,3]=="duskdawn")
points(plotsolar2[,1]~plotsolar2[,2],col=2)
