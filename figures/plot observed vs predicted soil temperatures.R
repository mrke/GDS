################################### plot C2 data against microclimate model #####################################################################
...
par(mfrow = c(2,1)) # set up for 6 plots in 2 columns
...
predicted<-read.csv('microclimate/soil.csv')
...
with(predicted,plot(D1.5cm~dates,type='l',col='red',ylim=c(-10,70),xlim=c(as.numeric(as.POSIXct("2013-09-01",origin="1970-01-01")),as.numeric(as.POSIXct("2014-10-01",origin="1970-01-01"))),main="modelled",xlab="",ylab="temperature (deg C)",xaxt = "n")) # plot the data
axis.POSIXct(side = 1, x = soil.surfaces$date_time,
             at = seq(ISOdate(2013,08,011), ISOdate(2014,11,01), "months"), format = "%b %y",
             las = 2)
with(predicted,points(D5cm~dates,type='l',col='orange')) # plot the data
with(predicted,points(D15cm~dates,type='l',col='brown')) # plot the data
with(predicted,points(D30cm~dates,type='l',col='light blue')) # plot the data
with(predicted,points(D60cm~dates,type='l',col='blue')) # plot the data
with(predicted,points(D100cm~dates,type='l',col='black')) # plot the data
