...
# run anovas
model = aov(temperature ~ depth + Error(month / (depth)), monthly.max)
summary(model)

max.summary = as.data.frame(aggregate(monthly.max$temperature, by=list(monthly.max$month, monthly.max$depth), FUN="mean"))
colnames(max.summary)=c("month", "depth", "temperature")
...
# save group means
max.table=cbind(seq(1,12),max.summary$temperature[1:12],max.summary$temperature[13:24],max.summary$temperature[25:36])
colnames(max.table)=c("month","deep", "mid", "surf")
write.csv(max.table,paste0(dropbox,"burrow chapter final scripts/summary stats/burrow_temps_max.csv"))
...
# run anovas
model = aov(temperature ~ depth + Error(month / (depth)), monthly.min)
summary(model)

min.summary = as.data.frame(aggregate(monthly.min$temperature, by=list(monthly.min$month, monthly.min$depth), FUN="mean"))
colnames(min.summary)=c("month", "depth", "temperature")
...
# save group means
min.table=cbind(seq(1,12),min.summary$temperature[1:12],min.summary$temperature[13:24],min.summary$temperature[25:36])
colnames(min.table)=c("month","deep", "mid", "surf")
write.csv(min.table,paste0(dropbox,"burrow chapter final scripts/summary stats/burrow_temps_min.csv"))
