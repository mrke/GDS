...
# histogram dawn
time_budget_duskdawn<-subset(time_budget,timeday=='dawn')
...
hist(x=time_budget_duskdawn_nongravid$Tb,breaks=brks,col='NA',border="NA",main="dawn",ylim=c(0,300))
...
# histogram dusk
time_budget_duskdawn<-subset(time_budget,timeday=='dusk')
time_budget_duskdawn_gravid<-subset(time_budget_duskdawn,gravid=='p')
time_budget_duskdawn_nongravid<-subset(time_budget_duskdawn,gravid=='np' | sex=='f')
time_budget_duskdawn_males<-subset(time_budget_duskdawn,sex=='m')
hist(x=time_budget_duskdawn_nongravid$Tb,breaks=brks,col='NA',border="NA",main="dusk",ylim=c(0,300))
hist(x=time_budget_duskdawn$c.shd,breaks=brks,col='black',border="NA",add=TRUE)
hist(x=time_budget_duskdawn$c.sun,breaks=brks,col='black',border="NA",add=TRUE)
hist(x=time_budget_duskdawn$c.part,breaks=brks,col='black',border="NA",add=TRUE)
hist(x=time_budget_duskdawn$s.5cm,breaks=brks,col='grey',border="NA",add=TRUE)
...
counts <- table(time_budget_id$state, time_budget_id$timeday)
order=c(3,4,5,1,2,6)
props <- prop.table(counts, margin = 2)
props = cbind(props,order)
props = props[order(props[,5]),][,-5]

barplot(props, ylim = c(0,1), cex.lab = 1.3, cex = 1.3, cex.axis =1.3, tck=0.01, yaxs = "i", ylab = "", main = paste0(lizards[i], " (",ndays[i]," days)"),
  xlab="", col=c("black", "dark grey", "red", "orange", "blue", "light blue") )
mtext(outer = TRUE, text = 'proportion of time',side = 2, line = -1)

box()  

}

legend("bottomright", pch = rep(15,6), inset=c(-1,-.1), col=rev(c("black", 
...
for(i in 1:length(lizards)){
  ...
  for(j in 1:length(alldays)){
    ...
    if(as.character(alldays[j]) == as.character(days[count]) & count <= length(days)){
      ...
      if(nrow(time_budget_day) > 0){  
        ...
        if(j >= start  & j <= finish){
          if(i==1){
          barplot(props, ylim = c(0,1), cex.lab = .8, cex = .8, cex.axis =.8, tck=0.01, yaxs = "i", ylab = "", main = days[count],
            xlab="", col=c("black", "dark grey", "red", "orange", "blue", "light blue") )
  ...
  xpd=NA, bty = 'n', cex = .85)
          ...
          barplot(props, ylim = c(0,1), cex.lab = .8, cex = .8, cex.axis =.8, tck=0.01, yaxs = "i", ylab = "",
            xlab="", col=c("black", "dark grey", "red", "orange", "blue", "light blue") )          
