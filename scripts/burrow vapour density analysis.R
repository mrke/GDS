...
# run anovas
model = aov(vd ~ depth + Error(month / (depth)), monthly.max)
summary(model)

max.summary = as.data.frame(aggregate(monthly.max$vd, by=list(monthly.max$month, monthly.max$depth), FUN="mean"))
colnames(max.summary)=c("month", "depth", "vd")
...
# save group means
max.table=cbind(seq(1,12),max.summary$vd[1:12],max.summary$vd[13:24],max.summary$vd[25:36])
colnames(max.table)=c("month","deep", "mid", "surf")
write.csv(max.table,paste0(dropbox,"burrow chapter final scripts/summary stats/burrow_vapdens_max.csv"))
...
# run anovas
model = aov(vd ~ depth + Error(month / (depth)), monthly.min)
summary(model)

min.summary = as.data.frame(aggregate(monthly.min$vd, by=list(monthly.min$month, monthly.min$depth), FUN="mean"))
colnames(min.summary)=c("month", "depth", "vd")
...
# save group means
min.table=cbind(seq(1,12),min.summary$vd[1:12],min.summary$vd[13:24],min.summary$vd[25:36])
colnames(min.table)=c("month","deep", "mid", "surf")
write.csv(min.table,paste0(dropbox,"burrow chapter final scripts/summary stats/burrow_vapdens_min.csv"))
