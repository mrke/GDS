...
with(mean.max.surf,plot(date,mean.max.humidity, cex.axis=1.2, xaxs = "i", type='l',col='white', ylim=c(0,110),xlim=c(as.numeric(as.POSIXct(datestart,origin="1970-01-01")),as.numeric(as.POSIXct(datefinish,origin="1970-01-01"))),xlab="",ylab="",xaxt = "n")) # plot the data
...
title(ylab="relative humidity (%)",mgp=c(2.3,1,0), cex.lab = 1.5)
...
box()
#polygon(c(max.weather_obs$TIMESTAMP,rev(min.weather_obs$TIMESTAMP)),c(max.weather_obs$humidity,rev(min.weather_obs$humidity)),col='light grey', border=NA)
#points(max.weather_obs$humidity~max.weather_obs$TIMESTAMP,type='l',col='blue', pch="+",lwd=1)
#points(min.weather_obs$humidity~min.weather_obs$TIMESTAMP,type='l',col='blue', pch="+",lwd=1)
